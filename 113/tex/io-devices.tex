...
\begin{tikzpicture} [
   ...
   \node (root) [text width=18em] {IO Memory Interface}
      child {node (pio) {Physical-Memory Mapping}}
      child {node(intio) {Separate-Memory Mapping}};
\end{tikzpicture}
\end{figure}
...
IA-32 supports both physical-memory mapping and seperate-memory mapping. 
...
\highlightdef{\textbf{Interrupts} are a \textit{calling 
mechanism} provided by the processor (and nearby chips) 
and given to \textsc{io} devices }
