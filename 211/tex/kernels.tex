...
Unix however, has the notion of \textit{special files}.
...
\begin{tikzpicture} [
   ...
   level 2 every node/.style={text width=0.1cm}
]
   \node 
   (root) {Unix Files}
      child {
      ...
      child {
        ...
        child {node(d) {\lstinline{d}}}
        child {node(l) {\lstinline{l}}}
        child {node(p) {\lstinline{p}}}
        child {node(c) {\lstinline{c}}}
        child {node(b) {\lstinline{b}}}
        child {node(s) {\lstinline{s}}}
      };
