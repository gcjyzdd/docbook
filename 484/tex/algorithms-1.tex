...
but are assuming that $f(x)$ is either \textit{balanced} or \textit{constant}.
It's not necessarily being a parity function as it was the case in the parity problem. 
There is no longer a $u$ to output, so the last part of the Bernstein-Vazirani Algorithm 
will be slightly different. We will adjust the derivation to solve the Balanced Problem.
...
\highlightdef{After measuring the first $n$ quibits, i f we observe $\ket{0^n}$, $f$ is \textit{definitely} constant, 
